...
def rasterize_points_square(
    ...
    return _RasterizePointsSquare.apply(
        points_packed,
...
class _RasterizePointsSquare(torch.autograd.Function):
    @staticmethod
    ...
    def forward(
        ...
        idx, zbuf, dists = _C.rasterize_points_square(*args)
        ctx.save_for_backward(points, idx, radius)
        ctx.mark_non_differentiable(idx, radius)
    ...
    def backward(ctx, grad_idx, grad_zbuf, grad_dists):
        ...
        args = (points, idx, radius, grad_zbuf, grad_dists)
        grad_points = _C.rasterize_points_square_backward(*args)
        grads = (
            grad_points,
...
def rasterize_points_square_python(
    pointclouds,
