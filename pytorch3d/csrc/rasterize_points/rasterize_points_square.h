...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> RasterizePointsSquareNaiveCpu(
...
RasterizePointsSquareNaiveCuda(
    const torch::Tensor& points,
    const torch::Tensor& cloud_to_packed_first_idx,
    const torch::Tensor& num_points_per_cloud,
    const std::tuple<int, int> image_size,
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> RasterizePointsSquareNaive(
    const torch::Tensor& points,
    const torch::Tensor& cloud_to_packed_first_idx,
    const torch::Tensor& num_points_per_cloud,
    const std::tuple<int, int> image_size,
  ...
  if (points.is_cuda() && cloud_to_packed_first_idx.is_cuda() &&
    ...
    CHECK_CUDA(radius);
    return RasterizePointsSquareNaiveCuda(
        points,
    ...
    return RasterizePointsSquareNaiveCpu(
        points,
        cloud_to_packed_first_idx,
        num_points_per_cloud,
        image_size,
...
torch::Tensor RasterizePointsSquareCoarseCpu(
    const torch::Tensor& points,
    const torch::Tensor& cloud_to_packed_first_idx,
    const torch::Tensor& num_points_per_cloud,
    const std::tuple<int, int> image_size,
...
torch::Tensor RasterizePointsSquareCoarse(
    const torch::Tensor& points,
    const torch::Tensor& cloud_to_packed_first_idx,
    const torch::Tensor& num_points_per_cloud,
    const std::tuple<int, int> image_size,
  ...
  if (points.is_cuda() && cloud_to_packed_first_idx.is_cuda() &&
    ...
    return RasterizePointsSquareCoarseCpu(
        points,
        cloud_to_packed_first_idx,
        num_points_per_cloud,
        image_size,
...
#ifdef WITH_CUDA
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> RasterizePointsSquareFineCuda(
    const torch::Tensor& points,
...
//         distance in the (NDC) x/y plane between each pixel and its K closest
//         points along the z axis.
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> RasterizePointsSquareFine(
    const torch::Tensor& points,
    const torch::Tensor& bin_points,
    const std::tuple<int, int> image_size,
    const torch::Tensor& radius,
  ...
  if (points.is_cuda()) {
    ...
    return RasterizePointsSquareFineCuda(
...
torch::Tensor RasterizePointsSquareBackwardCpu(
    const torch::Tensor& points,
...
#ifdef WITH_CUDA
torch::Tensor RasterizePointsSquareBackwardCuda(
    const torch::Tensor& points,
    const torch::Tensor& idxs,
    const torch::Tensor& radius,
    const torch::Tensor& grad_zbuf,
...
torch::Tensor RasterizePointsSquareBackward(
    const torch::Tensor& points,
    const torch::Tensor& idxs,
    const torch::Tensor& radius,
    const torch::Tensor& grad_zbuf,
  ...
  if (points.is_cuda()) {
    ...
    return RasterizePointsSquareBackwardCuda(points, idxs, radius, grad_zbuf, grad_dists);
#else
    AT_ERROR("Not compiled with GPU support");
#endif
  } else {
    return RasterizePointsSquareBackwardCpu(points, idxs, radius, grad_zbuf, grad_dists);
  }
}
...
//         distance in the (NDC) x/y plane between each pixel and its K closest
//         points along the z axis.
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> RasterizePointsSquare(
    const torch::Tensor& points,
    const torch::Tensor& cloud_to_packed_first_idx,
    const torch::Tensor& num_points_per_cloud,
    const std::tuple<int, int> image_size,
  ...
  if (bin_size == 0) {
    ...
    return RasterizePointsSquareNaive(
        points,
        cloud_to_packed_first_idx,
        num_points_per_cloud,
        image_size,
    ...
    const auto bin_points = RasterizePointsSquareCoarse(
        points,
        cloud_to_packed_first_idx,
        num_points_per_cloud,
        image_size,
        ...
        bin_size,
        max_points_per_bin);
    return RasterizePointsSquareFine(
        points, bin_points, image_size, radius, bin_size, points_per_pixel);
  }
}
