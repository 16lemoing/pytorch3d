...
// This code is shared between RasterizePointsSquareNaiveCudaKernel and
// RasterizePointsSquareFineCudaKernel.
...
__device__ void CheckPixelInsidePoint(
  ...
  const float adx = abs(xf - px);
  const float ady = abs(yf - py);
  if (adx < p_radius && ady < p_radius) {
    const float dist2 = 1 - (p_radius - adx) * (p_radius - ady) / radius2;
    if (q_size < K) {
      // Just insert it
...
__global__ void RasterizePointsSquareNaiveCudaKernel(
    const float* points, // (P, 3)
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> RasterizePointsSquareNaiveCuda(
  ...
  at::CheckedFrom c = "RasterizePointsSquareNaiveCuda";
  at::checkAllSameGPU(
  ...
  RasterizePointsSquareNaiveCudaKernel<<<blocks, threads, 0, stream>>>(
...
__global__ void RasterizePointsSquareFineCudaKernel(
    const float* points, // (P, 3)
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> RasterizePointsSquareFineCuda(
    const at::Tensor& points, // (P, 3)
  ...
  at::CheckedFrom c = "RasterizePointsSquareFineCuda";
  ...
  const size_t blocks = 1024;
  const size_t threads = 64;
  RasterizePointsSquareFineCudaKernel<<<blocks, threads, 0, stream>>>(
      points.contiguous().data_ptr<float>(),
...
__global__ void RasterizePointsSquareBackwardCudaKernel(
    const float* points, // (P, 3)
  ...
  for (int i = tid; i < N * H * W * K; i += num_threads) {
    ...
    const float p_radius = radius[p];
    const float radius2 = p_radius * p_radius;
    const float dx = px - xf;
    const float dy = py - yf;
    const float adx = abs(dx);
    const float ady = abs(dy);
    const float sdx = (dx > 0) - (dx < 0);
    const float sdy = (dy > 0) - (dy < 0);
    const float grad_px = sdx * grad_dist2 * ((p_radius - ady) / radius2);
    const float grad_py = sdy * grad_dist2 * ((p_radius - adx) / radius2);
...
at::Tensor RasterizePointsSquareBackwardCuda(
  ...
  at::CheckedFrom c = "RasterizePointsSquareBackwardCuda";
  ...
  RasterizePointsSquareBackwardCudaKernel<<<blocks, threads, 0, stream>>>(
      points.contiguous().data_ptr<float>(),
